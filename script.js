const API_KEY = '949258ff4ff329d48e662c7badcd4ac9'; // Replace with your TMDb API key
...
function displayMovies(movies) {
    ...
    movies.forEach(async movie => {
        ...
        let trailerEmbed = '';
        const trailerButton = `<button class="watch-button" onclick="toggleTrailer('${movie.id}')">Watch Trailer</button>`;
        
        if (trailer) {
            // If trailer exists, embed the video
            trailerEmbed = `
        ...
        movieElement.innerHTML = `
            ...
            ${trailerButton}  <!-- Watch Trailer Button -->
            ${trailerEmbed}  <!-- Embed the trailer if it exists -->
        `;
